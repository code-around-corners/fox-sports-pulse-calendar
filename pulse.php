	...
	*/
	...
	function fspc_main() {
		...
		if ( fspc_get_mode() == 0 ) {
			...
			$calendarData = fspc_cache_get($cacheUrl, 'ics');
			
			if ( ! $calendarData || isset($_GET["nocache"]) ) {
				$sportID = 0;
				$assocID = $_GET['assoc'];
				$clubID = $_GET['club'];
				$teamID = $_GET['team'];
				...
				$calendarData = fspc_cal_get_calendar($timecheck, $gamedata, $teamname, $timezone, $extdata, $exttz, $clash, $text);
				fspc_cache_set($cacheUrl, "ics", $calendarData, 86400);
			}

			$teamname = fspc_cache_get($sportID . '-' . $assocID . '-' . $clubID, 'teamname');

			if ( ! $teamname ) {
				$teamname = fspc_fsp_get_club_name($sportID, $assocID, $clubID);
				if ( $teamname == '' ) $teamname = $FSPC_DEFAULT_TEAM_NAME;

				fspc_cache_set($sportID . '-' . $assocID . '-' . $clubID, 'teamname', $teamname, 86400 * 30);
			}
